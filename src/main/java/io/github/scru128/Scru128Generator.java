...
public class Scru128Generator implements Iterable<@NotNull Scru128Id>, Iterator<@NotNull Scru128Id> {
     ...
     * detected, this method resets the generator state and returns a new ID based on the up-to-date timestamp.
     * <p>
    ...
    /**
     * Generates a new SCRU128 ID object from the current timestamp, guaranteeing that the returned ID is greater than
     * the immediately preceding one generated by the same generator.
     * <p>
     * This method returns monotonically increasing IDs unless the up-to-date timestamp is significantly (by ten seconds
     * or more) smaller than the one embedded in the immediately preceding ID. If such a significant clock rollback is
     ...
     * @return new SCRU128 ID object.
     */
    public synchronized @Nullable Scru128Id generateMonotonic() {
        return generateCoreMonotonic(System.currentTimeMillis());
    }
     ...
     * detected, this method resets the generator state and returns a new ID based on the given argument.
     * <p>
    ...
    public @NotNull Scru128Id generateCore(long timestamp) {
        Scru128Id value = generateCoreMonotonic(timestamp);
        if (value == null) {
            // reset state and resume
            this.timestamp = 0;
            tsCounterHi = 0;
            value = generateCoreMonotonic(timestamp);
    ...
    /**
     * Generates a new SCRU128 ID object from the timestamp passed, guaranteeing that the returned ID is greater than
     * the immediately preceding one generated by the same generator.
     * <p>
     * This method returns monotonically increasing IDs unless a given timestamp is significantly (by ten seconds or
     * more) smaller than the one embedded in the immediately preceding ID. If such a significant clock rollback is
     * detected, this method returns null and keeps the generator state untouched.
     * <p>
     * Unlike {@link #generateMonotonic()}, this method is NOT thread-safe. The generator object should be protected
     * from concurrent accesses using a mutex or other synchronization mechanism to avoid race conditions.
     *
     * @param timestamp 48-bit timestamp field value.
     * @return new SCRU128 ID object.
     * @throws IllegalArgumentException if the argument is not a 48-bit positive integer.
     */
    public @Nullable Scru128Id generateCoreMonotonic(long timestamp) {
